module Fastlane
  module Actions
    class CryptexGenerateKeystoreAction < Action
      def self.run(params)
        require "fileutils"
        cmd = "keytool -genkey -v -keyalg RSA -keysize 2048 -keystore #{File.expand_path(params[:destination])} -storepass #{params[:keystore_password]} -keypass #{params[:key_password]} -alias #{params[:alias]} -dname 'CN=#{params[:fullname]},L=#{params[:city]}' -validity 1000"
